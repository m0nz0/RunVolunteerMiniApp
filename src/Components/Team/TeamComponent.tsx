...
export const TeamComponent: FC = () => {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                let data = await TeamService.getTeam(
                    Number(locationId),
                    Number(calendarId),
                )
                setTeam(data)
    ...
    function thisPositions() {
        return team?.positions.filter(
            (p) =>
                p.positionType == PositionType.Main ||
                team?.schedules.map((s) => s.positionId).some((s) => s === p.id),
        )
    }

    const positionHierarchy = Object.entries(
        Object.groupBy(
            (team?.positions ?? []).filter(
                (p) =>
                    p.positionType == PositionType.Main ||
                    team?.schedules.map((s) => s.positionId).some((s) => s === p.id),
            ),
        ...
        .sort((a, b) => {
            ...
            let namea = team?.positions.find(
                (x) => x.id === Number(a?.position?.id),
            )?.name
            let nameb = team?.positions.find(
                (x) => x.id === Number(b?.position?.id),
            )?.name
    ...
    let canSchedule =
        !team?.schedules.some((x) => x.tgUser.tgId === userId) &&
        !team?.hasOtherLocations

    let canTrySaveNrms = team?.verstAthlete?.id != null

    const startPoll = async () => {
        try {
            await TeamService.startPoll(team?.pollParameters?.initiatorId).then(() =>
                toast.success('Перекличка запущена. Следите за результатами в боте', {
    ...
    const deleteItem = async (scheduleId: number) => {
        setDisabled(true)
        try {
            await TeamService.undoSchedule(scheduleId, UndoType.Admin)
    ...
    return (
        <div>
            <div className={'text-center'}>
                <h5>
                    Команда&nbsp;
                    {team?.lastEvent ? `${team.lastEvent.eventNumber} старта ` : ''}
            ...
            <div style={{display: 'flex', justifyContent: 'space-between'}}>
                <NameWithBadgeComponent
                    name={'Записалось волонтёров'}
                    badgeColor="success"
                    badgeValue={(team?.schedules ?? []).length}
                />
                <NameWithBadgeComponent
                    name={'Незакрытых позиций'}
                    badgeColor={'danger'}
                    badgeValue={
                        (thisPositions() ?? []).filter(
                            (p) => !team?.schedules.some((s) => s.positionId === p.id),
                        ).length
                    }
                />
                <NameWithBadgeComponent
                    name={'Частично закрыто позиций'}
                    badgeColor={'warning'}
                    badgeValue={
                        (positionHierarchy ?? []).filter(
                            (p) =>
            ...
            <ListGroup>
                {(positionHierarchy ?? []).map((x, index, array) => {
                    ...
                    if (x.subPositions.length == 0) {
                        ...
                        if (x.subPositions.every((sp) => sp.users?.length == 0)) {
                            ...
                            x.subPositions.every((sp) => (sp.users ?? []).length > 0)
                        ) {
                            rootColor = 'success'
                        } else {
                            rootColor = 'warning'
                        ...
                        >
                            <div>
                                <NameWithBadgeComponent
                                    ...
                                    badgeValue={
                                        (x?.users ?? []).length +
                                        (x.subPositions ?? []).flatMap((sp) => sp.users ?? [])
                                            .length
                                    }
                                ...
                                {(x.subPositions ?? []).map((sp) => (
                                    <div
                                        key={x.position.id + '-' + sp.position.id}
                                    ...
                                    >
                                        <NameWithBadgeComponent
                                            ...
                                            badgeColor={
                                                sp?.users?.length === 0 ? 'danger' : 'success'
                                            }
                                        />
                                        {(sp.users ?? []).map((u) => {
                                            return (
                ...
                <ButtonGroup vertical className={'gap-2'}>
                    {canSchedule &&
                        AppButtons.ToPositionFromTeam(
                            Number(locationId),
                            Number(calendarId),
                        )}
                    {team?.pollParameters.canCreate && (
                        <Button variant={'info'} size={'sm'} onClick={() => startPoll()}>
                            Запустить перекличку
                        </Button>
                    )}
                    {canTrySaveNrms &&
                        AppButtons.AuthNrms(Number(locationId), Number(calendarId))}
