...
export const EntryComponent: FC<Props> = (props: any) => {
    ...
    const handleUndo = async (item: Team) => {
        setDisabled(true)
        await TeamService.undoSchedule(item.id, UndoType.User)
            .then((value) => {
                toast.warn(
                    <p className={'text-center'}>
                        Ваша запись в волонтёры локации {item.location.name} на&nbsp;
                        {DateService.formatDayMonthNameYear(item.calendar.date)} на
                        позицию&nbsp;
                        {item.position.name} отменена
                        <br />
                        Очень жаль.
                        <br />
        ...
        <Card style={{cursor: 'pointer'}}>
            <Card.Body>
                ...
                <div>
                    <strong>Дата: </strong>
                    <span>
            {DateService.formatDayMonthNameYear(props.team.calendar.date)}
          </span>
                </div>
                <div>
            ...
            <Card.Footer>
                <ButtonToolbar className={'d-grip gap-2'}>
                    <ButtonGroup>
                        {AppButtons.ToTeamFromExistingDate(
                            props.team.locationId,
                            props.team.calendarId,
                            'Посмотреть команду',
                        )}
                    </ButtonGroup>
                    <ButtonGroup>
